...
/* Gigabit PHY has addr 1 and PCIe PHY is 2 */
#define E1000_GB_PHY_ADDR	1
...
/* These two defines should be changed simultenaously  */
#define E1000_RX_PKT_SIZE	2048
#define E1000_RCTL_SZ_DEF	E1000_RCTL_SZ_2048
#define E1000_RX_RING_SIZE	(E1000_RX_DESC_COUNT * sizeof(struct e1000_rx_desc))
#define E1000_RX_BUF_SIZE	(E1000_RX_DESC_COUNT * E1000_RX_PKT_SIZE)
#define E1000_RX_TOTAL_SIZE	(E1000_RX_RING_SIZE + E1000_RX_BUF_SIZE)

#define e1000_dbg(priv, args...) dev_dbg(&priv->pdev->dev, ## args)
#define e1000_err(priv, args...) dev_err(&priv->pdev->dev, ## args)
...
static int e1000_setup_link(struct eth_device *edev);
static int e1000_setup_fiber_link(struct eth_device *edev);
static int e1000_setup_copper_link(struct eth_device *edev);
static int e1000_phy_setup_autoneg(struct e1000_hw *hw);
static void e1000_config_collision_dist(struct e1000_hw *hw);
static int e1000_config_mac_to_phy(struct e1000_hw *hw);
static int e1000_config_fc_after_link_up(struct e1000_hw *hw);
static int e1000_check_for_link(struct eth_device *edev);
	...
	if (hw->mac_type != e1000_82573 || hw->mac_type != e1000_82574) {
		...
		if (hw->mac_type > e1000_82544) {
			...
			if (!(eecd & E1000_EECD_GNT)) {
				eecd &= ~E1000_EECD_REQ;
				E1000_WRITE_REG(hw, EECD, eecd);
				e1000_err(hw->priv, "Could not acquire EEPROM grant\n");
...
e1000_spi_eeprom_ready(struct e1000_hw *hw)
{
	...
	if (retry_count >= EEPROM_MAX_RETRY_SPI) {
		e1000_err(hw->priv, "SPI EEPROM Status error\n");
		return -E1000_ERR_EEPROM;
	}
		...
		uint16_t words, uint16_t *data)
{
	...
	if ((offset >= eeprom->word_size) ||
		...
		(words == 0)) {
		e1000_err(hw->priv, "\"words\" parameter out of bounds."
...
static int e1000_validate_eeprom_checksum(struct e1000_hw *hw)
{
	...
	if (!buf) {
		e1000_err(hw->priv, "Unable to allocate EEPROM buffer!\n");
		return -E1000_ERR_EEPROM;
	}

	/* Read the EEPROM */
	if (e1000_read_eeprom(hw, 0, EEPROM_CHECKSUM_REG + 1, buf) < 0) {
		e1000_err(hw->priv, "Unable to read EEPROM!\n");
		return -E1000_ERR_EEPROM;
	}
	...
	e1000_err(hw->priv, "EEPROM checksum is incorrect!\n"
...
e1000_get_software_semaphore(struct e1000_hw *hw)
{
	...
	if (!timeout) {
		e1000_err(hw->priv, "Driver can't access device - SMBI bit is set.\n");
...
e1000_get_hw_eeprom_semaphore(struct e1000_hw *hw)
{
	...
	if (!timeout) {
		...
		e1000_err(hw->priv, "Driver can't access the Eeprom - "
				"SWESMBI bit is set.\n");
		return -E1000_ERR_EEPROM;
	}
#endif
...
e1000_swfw_sync_acquire(struct e1000_hw *hw, uint16_t mask)
{
	...
	if (!timeout) {
		e1000_err(hw->priv, "Driver can't access resource, SW_FW_SYNC timeout.\n");
		return -E1000_ERR_SWFW_SYNC;
	}
...
static int e1000_get_ethaddr(struct eth_device *edev, unsigned char *mac_addr)
{
	...
	for (i = 0; i < NODE_ADDRESS_SIZE; i += 2) {
		...
		if (e1000_read_eeprom(hw, offset, 1, &eeprom_data) < 0) {
			e1000_err(priv, "EEPROM Read Error\n");
			return -E1000_ERR_EEPROM;
		}
		mac_addr[i] = eeprom_data & 0xff;
		mac_addr[i + 1] = (eeprom_data >> 8) & 0xff;
	}

	/* Invert the last bit if this is the second device */
	if (e1000_is_second_port(hw))
		mac_addr[NODE_ADDRESS_SIZE - 1] ^= 1;
#else
	memset(mac_addr, 0, NODE_ADDRESS_SIZE);
#endif

	e1000_dbg(priv, "Read mac %02x:%02x:%02x:%02x:%02x:%02x\n",
...
static int
e1000_set_ethaddr(struct eth_device *edev, unsigned char *mac_addr)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	uint32_t i;
	...
	e1000_dbg(hw->priv, "Programming MAC Address into RAR[0]\n");
	addr_low = (mac_addr[0] |
		    (mac_addr[1] << 8) |
		    (mac_addr[2] << 16) | (mac_addr[3] << 24));

	addr_high = (mac_addr[4] | (mac_addr[5] << 8) | E1000_RAH_AV);

	E1000_WRITE_REG_ARRAY(hw, RA, 0, addr_low);
	E1000_WRITE_REG_ARRAY(hw, RA, 1, addr_high);

	/* Zero out the other 15 receive addresses. */
	e1000_dbg(hw->priv, "Clearing RAR[1-15]\n");
...
e1000_set_mac_type(struct e1000_hw *hw)
{
	...
	e1000_dbg(hw->priv, "Mac type %d detected\n", hw->mac_type);
	return E1000_SUCCESS;
}
...
e1000_reset_hw(struct e1000_hw *hw)
{
	...
	uint32_t manc;
	uint32_t pba = 0;
	uint16_t pci_cmd_word = 0;

	e1000_dbg(hw->priv, "Resetting hardware\n");
	...
	if (hw->mac_type == e1000_82542_rev2_0) {
		pci_read_config_word(hw->priv->pdev, PCI_COMMAND, &pci_cmd_word);
		e1000_dbg(hw->priv, "Disabling MWI on 82542 rev 2.0\n");
		pci_write_config_word(hw->priv->pdev, PCI_COMMAND,
				pci_cmd_word & ~PCI_COMMAND_INVALIDATE);
	}

	/* Clear interrupt mask to stop board from generating interrupts */
	e1000_dbg(hw->priv, "Masking off all interrupts\n");
	 ...
	 */
	e1000_dbg(hw->priv, "Issuing a global reset to MAC\n");
	...
	if (hw->mac_type == e1000_82542_rev2_0) {
		pci_write_config_word(hw->priv->pdev, PCI_COMMAND, pci_cmd_word);
	}
...
e1000_initialize_hardware_bits(struct e1000_hw *hw)
{
	if ((hw->mac_type >= e1000_82571) &&
		...
		case e1000_82574:
			...
			reg_gcr = E1000_READ_REG(hw, GCR);
			reg_gcr |= (1 << 22);

			E1000_WRITE_REG(hw, GCR, reg_gcr);
 ...
 *****************************************************************************/
static int
e1000_init_hw(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	unsigned char mac_addr[NODE_ADDRESS_SIZE];
	uint32_t ctrl;
	uint32_t i;
	int32_t ret_val;
	...
	e1000_dbg(priv, "Initializing the IEEE VLAN\n");
	...
	if (hw->mac_type == e1000_82542_rev2_0) {
		pci_read_config_word(hw->priv->pdev, PCI_COMMAND, &pci_cmd_word);
		e1000_dbg(priv, "Disabling MWI on 82542 rev 2.0\n");
		pci_write_config_word(hw->priv->pdev, PCI_COMMAND,
				      pci_cmd_word & ~PCI_COMMAND_INVALIDATE);
	 ...
	 */
	e1000_get_ethaddr(edev, mac_addr);
	e1000_set_ethaddr(edev, mac_addr);
	...
	if (hw->mac_type == e1000_82542_rev2_0) {
		...
		E1000_WRITE_FLUSH(hw);
		mdelay(1);
		pci_write_config_word(hw->priv->pdev, PCI_COMMAND, hw->pci_cmd_word);
	}

	/* Zero out the Multicast HASH table */
	e1000_dbg(priv, "Zeroing the MTA\n");
	...
	if (hw->bus_type == e1000_bus_type_pcix) {
		pci_read_config_word(hw->priv->pdev, PCIX_COMMAND_REGISTER,
				     &pcix_cmd_word);
		pci_read_config_word(hw->priv->pdev, PCIX_STATUS_REGISTER_HI,
				     &pcix_stat_hi_word);
		cmd_mmrbc =
		...
		if (cmd_mmrbc > stat_mmrbc) {
			...
			pci_write_config_word(hw->priv->pdev, PCIX_COMMAND_REGISTER,
					      pcix_cmd_word);
		}
	}
		break;
 ...
 *****************************************************************************/
static int
e1000_setup_link(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	int32_t ret_val;
	...
	if (e1000_read_eeprom(hw, EEPROM_INIT_CONTROL2_REG, 1,
				&eeprom_data) < 0) {
		e1000_dbg(priv, "EEPROM Read Error\n");
		return -E1000_ERR_EEPROM;
	}
#endif
	...
	e1000_dbg(priv, "After fix-ups FlowControl is now = %x\n", hw->fc);
	...
	ret_val = (hw->media_type == e1000_media_type_fiber) ?
	    e1000_setup_fiber_link(edev) : e1000_setup_copper_link(edev);
	if (ret_val < 0) {
		return ret_val;
	}
	 ...
	 */
	e1000_dbg(priv, "Initializing the Flow Control address, type "
			"and timer regs\n");
 ...
 *****************************************************************************/
static int
e1000_setup_fiber_link(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	uint32_t ctrl;
	...
	case e1000_fc_full:
		...
		break;
	default:
		e1000_dbg(priv, "Flow control param set incorrectly\n");
	 ...
	 */
	e1000_dbg(priv, "Auto-negotiation enabled (%#x)\n", txcw);
	...
	if ((E1000_READ_REG(hw, CTRL) & E1000_CTRL_SWDPIN1) == signal) {
		e1000_dbg(priv, "Looking for Link\n");
		...
		if (i == (LINK_UP_TIMEOUT / 10)) {
			 ...
			 */
			e1000_dbg(priv, "Never got a valid link from auto-neg!!!\n");
			hw->autoneg_failed = 1;
			ret_val = e1000_check_for_link(edev);
			if (ret_val < 0) {
				e1000_err(priv, "Error while checking for link\n");
				return ret_val;
			}
			hw->autoneg_failed = 0;
		} else {
			hw->autoneg_failed = 0;
			e1000_dbg(priv, "Valid Link Found\n");
		}
	} else {
		e1000_err(priv, "No Signal Detected\n");
...
e1000_copper_link_preconfig(struct e1000_hw *hw)
{
	...
	if (ret_val) {
		e1000_err(hw->priv, "Error, did not detect valid phy.\n");
		return ret_val;
	}
	e1000_dbg(hw->priv, "Phy ID = %x \n", hw->phy_id);
...
e1000_copper_link_igp_setup(struct e1000_hw *hw)
{
	...
	if (ret_val) {
		e1000_err(hw->priv, "Error Resetting the PHY\n");
		return ret_val;
	}
	...
	if (hw->phy_type == e1000_phy_igp) {
		...
		if (ret_val) {
			e1000_err(hw->priv, "Error Disabling LPLU D3\n");
			return ret_val;
		}
	}
	...
	if (ret_val) {
		e1000_err(hw->priv, "Error Disabling LPLU D0\n");
		return ret_val;
	}
...
e1000_copper_link_autoneg(struct e1000_hw *hw)
{
	...
	e1000_dbg(hw->priv, "Reconfiguring auto-neg advertisement params\n");
	ret_val = e1000_phy_setup_autoneg(hw);
	if (ret_val) {
		e1000_err(hw->priv, "Error Setting up Auto-Negotiation\n");
		return ret_val;
	}
	e1000_dbg(hw->priv, "Restarting Auto-Neg\n");
	...
	if (hw->wait_autoneg_complete) {
		...
		if (ret_val) {
			e1000_dbg(hw->priv, "Error while waiting for autoneg"
					"to complete\n");
			return ret_val;
		}
	}
...
e1000_copper_link_postconfig(struct e1000_hw *hw)
{
	...
	if (hw->mac_type >= e1000_82544) {
		...
		if (ret_val) {
			e1000_dbg(hw->priv, "Error configuring MAC to PHY settings\n");
			return ret_val;
		}
	}
	ret_val = e1000_config_fc_after_link_up(hw);
	if (ret_val) {
		e1000_dbg(hw->priv, "Error Configuring Flow Control\n");
		return ret_val;
	}
	return E1000_SUCCESS;
}
...
******************************************************************************/
static int
e1000_setup_copper_link(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	int32_t ret_val;
	...
	for (i = 0; i < 10; i++) {
		...
		if (phy_data & MII_SR_LINK_STATUS) {
			...
			e1000_dbg(hw->priv, "Valid link established!!!\n");
			return E1000_SUCCESS;
		}
		udelay(10);
	}

	e1000_err(hw->priv, "Unable to establish link!!!\n");
	return E1000_SUCCESS;
}
...
e1000_phy_setup_autoneg(struct e1000_hw *hw)
{
	...
	e1000_dbg(hw->priv, "autoneg_advertised %x\n", hw->autoneg_advertised);

	/* Do we want to advertise 10 Mb Half Duplex? */
	if (hw->autoneg_advertised & ADVERTISE_10_HALF) {
		e1000_dbg(hw->priv, "Advertise 10mb Half duplex\n");
		mii_autoneg_adv_reg |= NWAY_AR_10T_HD_CAPS;
	}

	/* Do we want to advertise 10 Mb Full Duplex? */
	if (hw->autoneg_advertised & ADVERTISE_10_FULL) {
		e1000_dbg(hw->priv, "Advertise 10mb Full duplex\n");
		mii_autoneg_adv_reg |= NWAY_AR_10T_FD_CAPS;
	}

	/* Do we want to advertise 100 Mb Half Duplex? */
	if (hw->autoneg_advertised & ADVERTISE_100_HALF) {
		e1000_dbg(hw->priv, "Advertise 100mb Half duplex\n");
		mii_autoneg_adv_reg |= NWAY_AR_100TX_HD_CAPS;
	}

	/* Do we want to advertise 100 Mb Full Duplex? */
	if (hw->autoneg_advertised & ADVERTISE_100_FULL) {
		e1000_dbg(hw->priv, "Advertise 100mb Full duplex\n");
		mii_autoneg_adv_reg |= NWAY_AR_100TX_FD_CAPS;
	}

	/* We do not allow the Phy to advertise 1000 Mb Half Duplex */
	if (hw->autoneg_advertised & ADVERTISE_1000_HALF) {
		e1000_dbg(hw->priv, "Advertise 1000mb Half duplex requested, request denied!\n");
	}

	/* Do we want to advertise 1000 Mb Full Duplex? */
	if (hw->autoneg_advertised & ADVERTISE_1000_FULL) {
		e1000_dbg(hw->priv, "Advertise 1000mb Full duplex\n");
	...
	case e1000_fc_full:	/* 3 */
		...
		break;
	default:
		e1000_err(hw->priv, "Flow control param set incorrectly\n");
		return -E1000_ERR_CONFIG;
	}
	...
	e1000_dbg(hw->priv, "Auto-Neg Advertising %x\n", mii_autoneg_adv_reg);
...
e1000_config_mac_to_phy(struct e1000_hw *hw)
{
	...
	if (e1000_read_phy_reg(hw, M88E1000_PHY_SPEC_STATUS, &phy_data) < 0) {
		e1000_err(hw->priv, "PHY Read Error\n");
...
e1000_config_fc_after_link_up(struct e1000_hw *hw)
{
	...
	if (((hw->media_type == e1000_media_type_fiber) && (hw->autoneg_failed))
		...
		if (ret_val < 0) {
			e1000_err(hw->priv, "Error forcing flow control settings\n");
			return ret_val;
		}
	}
		...
		if (mii_status_reg & MII_SR_AUTONEG_COMPLETE) {
			...
			if ((mii_nway_adv_reg & NWAY_AR_PAUSE) &&
				...
				if (hw->original_fc == e1000_fc_full) {
					hw->fc = e1000_fc_full;
					e1000_dbg(hw->priv, "Flow Control = FULL.\r\n");
				} else {
					hw->fc = e1000_fc_rx_pause;
					e1000_dbg(hw->priv, "Flow Control = RX PAUSE frames only.\r\n");
				}
			}
			...
			{
				hw->fc = e1000_fc_tx_pause;
				e1000_dbg(hw->priv, "Flow Control = TX PAUSE frames only.\r\n");
			}
			 ...
			 */
				...
				hw->fc = e1000_fc_none;
				e1000_dbg(hw->priv, "Flow Control = NONE.\r\n");
			} else {
				hw->fc = e1000_fc_rx_pause;
				e1000_dbg(hw->priv, "Flow Control = RX PAUSE frames only.\r\n");
			}
			...
			ret_val = e1000_force_mac_fc(hw);
			if (ret_val < 0) {
				e1000_dbg(hw->priv, "Error forcing flow control settings\n");
				return ret_val;
			}
		} else {
			e1000_dbg(hw->priv, "Copper PHY and Auto Neg has not completed.\r\n");
		}
	}
	return E1000_SUCCESS;
}
 ...
 *****************************************************************************/
static int
e1000_check_for_link(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	uint32_t rxcw;
	...
	e1000_dbg(priv, "ctrl: %#08x status %#08x rxcw %#08x\n", ctrl, status, rxcw);
	...
	if ((hw->media_type == e1000_media_type_copper) && hw->get_link_status) {
		...
		if (hw->mac_type >= e1000_82544)
			...
			ret_val = e1000_config_mac_to_phy(hw);
			if (ret_val < 0) {
				e1000_err(hw->priv, "Error configuring MAC to PHY settings\n");
				return ret_val;
			}
		}
		...
		ret_val = e1000_config_fc_after_link_up(hw);
		if (ret_val < 0) {
			e1000_err(hw->priv, "Error configuring flow control\n");
			return ret_val;
		}
	 ...
	 */
		...
		e1000_dbg(priv, "NOT RXing /C/, disable AutoNeg and force link.\r\n");
		...
		ret_val = e1000_config_fc_after_link_up(hw);
		if (ret_val < 0) {
			e1000_err(priv, "Error configuring flow control\n");
			return ret_val;
		}
	}
		...
		uint16_t *duplex)
{
	...
	if (hw->mac_type >= e1000_82543) {
		...
		if (status & E1000_STATUS_SPEED_1000) {
			*speed = SPEED_1000;
			e1000_dbg(hw->priv, "1000 Mbs, ");
		} else if (status & E1000_STATUS_SPEED_100) {
			*speed = SPEED_100;
			e1000_dbg(hw->priv, "100 Mbs, ");
		} else {
			*speed = SPEED_10;
			e1000_dbg(hw->priv, "10 Mbs, ");
		}

		if (status & E1000_STATUS_FD) {
			*duplex = FULL_DUPLEX;
			e1000_dbg(hw->priv, "Full Duplex\r\n");
		} else {
			*duplex = HALF_DUPLEX;
			e1000_dbg(hw->priv, " Half Duplex\r\n");
		}
	} else {
		e1000_dbg(hw->priv, "1000 Mbs, Full Duplex\r\n");
		*speed = SPEED_1000;
		*duplex = FULL_DUPLEX;
	}
...
e1000_wait_autoneg(struct e1000_hw *hw)
{
	...
	e1000_dbg(hw->priv, "Waiting for Auto-Neg to complete.\n");
	...
	for (i = PHY_AUTO_NEG_TIME; i > 0; i--) {
		...
		if (phy_data & MII_SR_AUTONEG_COMPLETE) {
			e1000_dbg(hw->priv, "Auto-Neg complete.\n");
			return 0;
		}
		mdelay(100);
	}
	e1000_err(hw->priv, "Auto-Neg timedout.\n");
...
e1000_read_phy_reg(struct e1000_hw *hw, uint32_t reg_addr, uint16_t * phy_data)
{
	...
	if (reg_addr > MAX_PHY_REG_ADDRESS) {
		e1000_err(hw->priv, "PHY Address %d is out of range\n", reg_addr);
	...
	if (hw->mac_type > e1000_82543) {
		...
		if (!(mdic & E1000_MDIC_READY)) {
			e1000_err(hw->priv, "MDI Read did not complete\n");
			return -E1000_ERR_PHY;
		}
		if (mdic & E1000_MDIC_ERROR) {
			e1000_err(hw->priv, "MDI Error\n");
			return -E1000_ERR_PHY;
		}
...
e1000_write_phy_reg(struct e1000_hw *hw, uint32_t reg_addr, uint16_t phy_data)
{
	...
	if (hw->mac_type > e1000_82543) {
		...
		if (!(mdic & E1000_MDIC_READY)) {
			e1000_err(hw->priv, "MDI Write did not complete\n");
			return -E1000_ERR_PHY;
		}
	} else {
...
e1000_get_phy_cfg_done(struct e1000_hw *hw)
{
	...
	case e1000_82572:
		...
		if (!timeout) {
			e1000_dbg(hw->priv, "MNG configuration cycle has not "
					"completed.\n");
			return -E1000_ERR_RESET;
		}
		break;
...
e1000_phy_hw_reset(struct e1000_hw *hw)
{
	...
	e1000_dbg(hw->priv, "Resetting Phy...\n");
	...
	if (hw->mac_type > e1000_82543) {
		...
		if (e1000_swfw_sync_acquire(hw, swfw)) {
			e1000_err(hw->priv, "Unable to acquire swfw sync\n");
			return -E1000_ERR_SWFW_SYNC;
		}
...
e1000_detect_gig_phy(struct e1000_hw *hw)
{
	...
	case e1000_ich8lan:
		...
		break;
	default:
		e1000_err(hw->priv, "Invalid MAC type %d\n", hw->mac_type);
		return -E1000_ERR_CONFIG;
	}
	...
	if ((match) && (phy_init_status == E1000_SUCCESS)) {
		e1000_dbg(hw->priv, "PHY ID 0x%X detected\n", hw->phy_id);
		return 0;
	}
	e1000_err(hw->priv, "Invalid PHY ID 0x%X\n", hw->phy_id);
	return -E1000_ERR_PHY;
}
...
static int
e1000_sw_init(struct e1000_priv *priv)
{
	struct e1000_hw *hw = &priv->hw;
	int result;

	/* PCI config space info */
	pci_read_config_byte(priv->pdev, PCI_REVISION_ID, &hw->revision_id);

	/* identify the MAC */
	result = e1000_set_mac_type(hw);
	if (result) {
		e1000_err(priv, "Unknown MAC Type\n");
		return result;
	}
	...
	if (hw->mac_type >= e1000_82543) {
		...
		if (status & E1000_STATUS_TBIMODE) {
			e1000_dbg(priv, "fiber interface detected\n");
			hw->media_type = e1000_media_type_fiber;
		} else {
			e1000_dbg(priv, "copper interface detected\n");
			hw->media_type = e1000_media_type_copper;
		}
	} else {
		hw->media_type = e1000_media_type_fiber;
 ...
 * hw - Struct containing variables accessed by shared code
 *****************************************************************************/
static void e1000_get_bus_type(struct e1000_hw *hw)
{
	uint32_t status;
	...
	e1000_dbg(hw->priv, "Found bus type %d\n", hw->bus_type);
}


static int e1000_phy_read(struct mii_bus *bus, int phy_addr, int reg)
{
	struct e1000_priv *priv = bus->priv;
	uint16_t phy_data;

	if (phy_addr != E1000_GB_PHY_ADDR)
		return 0xffff;

	if (e1000_read_phy_reg(&priv->hw, reg, &phy_data) != 0)
		return 0xffff;
	...
	int reg, u16 val)
{
	...
	if (e1000_write_phy_reg(&priv->hw, reg, val) != 0)
		return 1;

	return 0;
}

static void e1000_configure_hw_tx(struct e1000_hw *hw)
{
	unsigned long tctl;
...
static void
e1000_configure_tx(struct e1000_priv *priv)
{
	struct e1000_hw *hw = &priv->hw;
	unsigned int i;
...
e1000_setup_rctl(struct e1000_hw *hw)
{
	...
	rctl |= E1000_RCTL_SZ_DEF;

	/* Non-extended format and long packet disable */
 ...
 **/
static void
e1000_configure_rx(struct e1000_priv *priv)
{
	...
	E1000_WRITE_REG(hw, RDBAL, (u32) priv->rx_buffer_dma);
	E1000_WRITE_REG(hw, RDBAH, 0);

	E1000_WRITE_REG(hw, RDLEN, E1000_RX_RING_SIZE);

	/* Setup the HW Rx Head and Tail Descriptor Pointers */
	E1000_WRITE_REG(hw, RDH, 0);
	E1000_WRITE_REG(hw, RDT, E1000_RX_DESC_COUNT - 1);
...
static int e1000_eth_recv(struct eth_device *edev)
{
...
static int e1000_eth_send(struct eth_device *edev, void *packet,
				int packet_length)
{
	struct e1000_priv *priv = edev->priv;
	...
	while (!is_timeout(start, 10 * MSECOND)) {
		head = E1000_READ_REG(hw, TDH);
		if (head == priv->tx_tail) {
			goto wait_dd;
		}
	}

	e1000_dbg(priv, "TX did not handled descriptor %p\n", tx_desc);

	return 1;

wait_dd:
	start = get_time_ns();
	e1000_dbg(priv, "TX is handling descriptor %p\n", tx_desc);
	...
	e1000_dbg(priv, "TX failed to send descriptor %p\n", tx_desc);

	return 1;
}

static inline int
 e1000_reset(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	...
	return e1000_init_hw(edev);
}

static void
e1000_eth_halt(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
...
static int e1000_eth_open(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;
	int ret_val;
	...
	if (ret_val < 0) {
		if ((ret_val == -E1000_ERR_NOLINK) ||
			...
			e1000_err(priv, "Valid Link not detected\n");
		} else {
			e1000_err(priv, "Hardware Initialization Failed\n");
		}
		return 1;
	}
...
static int
e1000_eth_init(struct eth_device *edev)
{
	struct e1000_priv *priv = edev->priv;
	struct e1000_hw *hw = &priv->hw;

	/* MAC and Phy settings */
	if (e1000_sw_init(priv) < 0) {
		e1000_err(priv, "Software init failed\n");
		return -1;
	}
	if (e1000_check_phy_reset_block(hw))
		e1000_err(priv, "PHY Reset is blocked!\n");

	/* Basic init was OK, reset the hardware and allow SPI access */
	e1000_reset_hw(hw);

#if !defined(CONFIG_E1000_NO_NVM)
	if (e1000_init_eeprom_params(hw)) {
		e1000_err(priv, "EEPROM is invalid!\n");
		return -1;
	}
...
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	priv = xzalloc(sizeof(struct e1000_priv));

	priv->pdev = pdev;
	edev = &priv->edev;

	priv->miibus.read = e1000_phy_read;
	priv->miibus.write = e1000_phy_write;
	...
	hw->autoneg = 1;
	hw->get_link_status = true;
	hw->hw_addr = pci_iomap(priv->pdev, 0);
	hw->device_id = id->device;

	edev->init = e1000_eth_init;
	edev->halt = e1000_eth_halt;
	edev->send = e1000_eth_send;
	edev->recv = e1000_eth_recv;
	edev->open = e1000_eth_open;
